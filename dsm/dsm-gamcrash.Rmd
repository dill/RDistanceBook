...
If we have a model with the $s(x)$ given above in addition to $x$ and an intercept, we have the following design matrix (for $n$ samples):
$$
...
par(mfrow=c(1, 2)) # set up the plot window
plot(xy_dsm)
```

```{r plotsmooth_xy, fig.cap="Plot of the smooths of Northing and Easting on the Guld of Mexico dolphins.", fig.width=10}
par(mfrow=c(1,2))
plot(xy_dsm)
```
...
We now see that there is only one smooth term, `s(x,y)`. We can plot this effect in 2 dimensions using a heatmap (we use `scheme=1` to produce the heatmap rather than the other, harder to read options). We can also overplot the locations of the non-zero counts.

```{r plotsmooth-bi-noeval, eval=FALSE, echo=TRUE}
plot(xy_bi_dsm, scheme=1)
points(xy_bi_dsm$data[, c("x","y")][xy_bi_dsm$data$count>0, ])
```

```{r plotsmooth-bi-xy, fig.cap="Plot of the bivariate smooth of Northing and Easting on the Guld of Mexico dolphins. Heatmap colours indicate that red is lower values, white higher. Dots indicate observation locations (segments) and circles the location of non-zero counts.", fig.width=10}
plot(xy_bi_dsm, scheme=2)
points(xy_bi_dsm$data[, c("x","y")][xy_bi_dsm$data$count>0, ])
```
