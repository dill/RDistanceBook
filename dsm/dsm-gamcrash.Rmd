...
We talk about "additive terms" above and in the previous chapter we started thinking about smooths, smoothers and "wiggles". Now let's get into a bit more detail and think about how to specify and fit the covariate effects in our models.

Let's start by thinking about the good old statistical workhorse, the linear model. Figure XXXX shows a plot of some data, the left panel shows a linear fit through the data, the right shows a linear fit (`y~x`) with an additional term: the explanatory variable squared (`y~x+poly(x, 2)`). This seems to give a good fit too. The true function that generates this data is $\exp(2x)$, so the latter is a better model, as it has some curvature (though we're cheating here as we know about the data generation process).
...
```{r basis-ex, results='hide', fig.width=10, fig.width=5, fig.cap="Eight basis functions (grey, dashed lines) used to build a spline or smooth (solid line)."}
# generate some data
set.seed(2)
...
```{r first-dsm-double-basis, message=FALSE, warning=FALSE, echo=TRUE}
double_k_dsm <- dsm(formula=count~s(y, k=20), family=tw(), ddf.obj=dolphin_df,
                    segment.data=segdata, observation.data=obsdata)
```


```{r plotsmooth, fig.cap="Plot of the effect of Northing on the Guld of Mexico dolphins.", fig.width=10}
par(mfrow = c(1,2))
plot(simple_dsm, main="Smooth of x, k default")
plot(double_k_dsm, main="Smooth of x, k=20")
```

we can see that this changed the number of coefficients and the size of the penalty matrix:
...
We have the information we just extracted plus more, including the Tweedie parameter, the EDF of each term in the model as well as some information on significance, deviance and adjusted $R^2$. Note that the model formula includes a term `offset(off.set)` this is the $a_j\hat{p}_j$ in the formula (R needs to know it's an offset, and therefore has no coefficient to estimate, so it gets encased in `offset()`). We'll explore more of the `summary()` output as we continue looking at GAMs. `summary()` is usually the first step in investigating the model results.
...
xy_dsm <- dsm(formula=count~s(y) + s(x), family=tw(), ddf.obj=dolphin_df,
                    segment.data=segdata, observation.data=obsdata)
```
...
```{r plotsmooth_xy, fig.cap="Plot of the smooths of Northing and Easting on the Guld of Mexico dolphins. Note that setting `shade=TRUE` means our +/- 2 standard error band is shaded gray rather than delimited with dotted lines, this can be easier to see on plots with many elements.", fig.width=10}
par(mfrow=c(1,2))
plot(xy_dsm, shade=TRUE)
```
...
We now see that there is only one smooth term, `s(x,y)`. We can plot this effect in 2 dimensions using a heatmap (we use `scheme=1` to produce the heatmap rather than the other, harder to read options; the `asp=1` sets the $x$-$y$ aspect ratio to be one). We can also overplot the locations of the non-zero counts.
...
```{r plotsmooth-bi-xy, fig.cap="Plot of the bivariate smooth of Northing and Easting on the Guld of Mexico dolphins. Heatmap colours indicate that red is lower values, white higher. Dots indicate observation locations (segments) and circles the location of non-zero counts. Again this plot is on the linear predictor (logarithmic) scale.", fig.width=10}
