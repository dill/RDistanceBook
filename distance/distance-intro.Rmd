...
strip <- data.frame(x=c(-0.0125,-0.0125,0.0125,0.0125,-0.0125),y=c(0,1,1,0,0))
for(i in 1:5){
...
# generate some lines
# in this case we don't randomise the offset of the grid
lt <- data.frame(x=c(-0.0125,-0.0125,0.0125,0.0125,-0.0125),y=c(0,1,1,0,0))
# set sigma
sigma <- 0.01
...
mu <- integrate(function(x)  exp(-x^2/(2*0.01^2)), lower=0,upper=0.02)$value
# calculate p = mu/w
p <- mu/0.02
