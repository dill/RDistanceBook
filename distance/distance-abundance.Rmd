...
The analysis here is based on examples originally written by Eric Rexstad[^earstrata].
...
As before, we supply the fitted detection function object and a formula. This time the formula is `~Region.Label`, which separates the Northern and Southern regions of the study area.

```{r minke-df-abund}
abundance_estimate(minke_hn, ~Region.Label)
```

As a comparison we can set the total area and look at what the abundance would be estimated as if we considered a non-stratified analysis:
...
[^averagep]: Here, as elsewhere, "average detectability" and "average probability of detection are used as synonyms. Though the latter is more precise, the fomer is much less unwieldy.
