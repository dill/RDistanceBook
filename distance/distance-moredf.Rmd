...
Both the half-normal and hazard-rate detection functions are somewhat limited in their flexibility. A simple adaptation of these detection funcions is to consider them to be "key" functions which can be augmented with one or more "adjustment" terms [@Buckland:1992wy; @Buckland:2001vm]. The general form of these models mathematically is:
$$
g(y) = \frac{k(y)\left[ 1 + \sum_{j=2}^J \alpha_j a_j(y)\right]}{k(0)\left[ 1 + \sum_{j=2}^J \alpha_j a_j(0)\right]}
$$
where we are modelling our detection function $g(y)$ with the key function $k(y)$ (either half-normal or hazard-rate) and $J$ adjustment functions $a_j$ which are multiplied by corresponding coefficents $\alpha_j$ to be estimated. The denominator (which is simply the function evaluated at zero distance) ensures that the detection probability is certain at zero distance ($g(0)=1$). Different adjustments functions are considered to be appropriate for each key function.
...
```{r hn-adj-dolphins-plot, fig.width=10, fig.height=4, echo=FALSE, fig.cap="Detection function models fitted to the Mexico dolphin data, left to right: half-normal, half-normal with cosine adjustment of order 2, half-normal with Hermite polynomials of order 2."}
...
Second, the half-normal with cosine model begins by decreasing but then ends up increasing again before finally decreasing. We refer to this behaviour as *non-monotonic* and would prefer our detection functions to be *monotonically decreasing*; that is, as the distance from the observer increases we should see fewer animals. This makes sense as we are much more likely to see animals directly infront of us than those which are further away. If fitted detection functions exhibit this kind of monotonicity then it
...
```{r hr-adj-dolphins-plot, fig.width=10, fig.height=4, echo=FALSE, fig.cap="Detection function models fitted to the Mexico dolphin data, left to right: hazard-rate, half-normal with cosine adjustment of order 2, half-normal with simple polynomial of order 2."}
...
```{r unif-adj-dolphins-plot, fig.width=7, fig.height=4, echo=FALSE, fig.cap="Detection function models fitted to the Mexico dolphin data. Left: uniform with cosine adjustment of order 1. Right: uniform with cosine adjustments of order 1 and 2."}
par(mfrow=c(1,2))
...
So far we've looked at some new options for modelling the detection function. They are summarised in the folowing table:
...
Fitting a mixture model detection function is as straightforward as the K+A models described above:
```{r hn-mix-dolphins}
# 2-point mixture model
dolphins.hn_mix2 <- ds(mexdolphins,truncation=8000, df(model=~hn+mix(2)))
# 3-point mixture model
dolphins.hn_mix3 <- ds(mexdolphins,truncation=8000, df(model=~hn+mix(3)))
# 2-point including covariates
dolphins.hn_mix2.beau <- ds(mexdolphins,truncation=8000,
                            df(model=~hn+mix(2), formula=~beaufort))
```
...
talk about how covariates + adjustments are overparametrised
...
[^order]: Note that we refer to $j$ as the order, rather than $2j$ so we can be consittent over different adjustment functions.
